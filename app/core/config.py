...
# 默认走阿里云百炼 OpenAI 兼容接口，线上无需再依赖 DeepSeek 配置。
HARDCODED_PRIMARY_LLM_PROVIDER = "dashscope"
HARDCODED_PRIMARY_LLM_MODEL = "qwen3-max"
HARDCODED_DASHSCOPE_API_KEY = "sk-57a7c48444c74ccc8173024d9288e625"
HARDCODED_DASHSCOPE_BASE_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1"
...
DEEPSEEK_API_KEY = os.getenv(
    "DEEPSEEK_API_KEY",
    os.getenv("DASHSCOPE_API_KEY", HARDCODED_DASHSCOPE_API_KEY),
).strip()
DEEPSEEK_BASE_URL = os.getenv("DEEPSEEK_BASE_URL", HARDCODED_DASHSCOPE_BASE_URL)
DEEPSEEK_CHAT_MODEL = os.getenv("DEEPSEEK_CHAT_MODEL", HARDCODED_PRIMARY_LLM_MODEL)
DEEPSEEK_BASE_URL_FALLBACK = os.getenv("DEEPSEEK_BASE_URL_FALLBACK", HARDCODED_DASHSCOPE_BASE_URL)
...
DASHSCOPE_API_KEY = os.getenv("DASHSCOPE_API_KEY", HARDCODED_DASHSCOPE_API_KEY)
...
SYNDROME_RULES = {
    "痰热扰心": {
        "symptoms": ["失眠", "多梦", "心烦", "口苦", "口干", "痰多", "胸闷"],
        "tongue_tags": ["舌红", "苔黄腻"],
        "pulse_tags": ["脉滑", "滑数", "脉数"],
        "therapy": "清热化痰，宁心安神",
        "formula": "温胆汤加减",
        "mods": ["心烦重加黄连、栀子", "失眠重加酸枣仁、夜交藤", "口干明显加麦冬、天花粉"],
    },
    "心火上炎": {
        "symptoms": ["失眠", "心烦", "口干", "口苦", "小便黄"],
        "tongue_tags": ["舌红"],
        "pulse_tags": ["脉数", "滑数"],
        "therapy": "清心泻火，养阴安神",
        "formula": "导赤散合黄连阿胶汤加减",
        "mods": ["心烦重加黄连、栀子", "口干重加麦冬、生地", "小便黄重加淡竹叶、通草"],
    },
    "阴虚火旺": {
        "symptoms": ["失眠", "心烦", "口干", "盗汗", "多梦"],
        "tongue_tags": ["舌红", "苔薄"],
        "pulse_tags": ["脉细", "脉数"],
        "therapy": "滋阴降火，养心安神",
        "formula": "天王补心丹加减",
        "mods": ["口干重加麦冬、玄参", "盗汗重加浮小麦、牡蛎", "烦躁重加黄连、丹皮"],
    },
    "胃热伤津": {
        "symptoms": ["口干", "口渴", "口苦", "大便干", "喜冷饮"],
        "tongue_tags": ["舌红", "苔黄腻"],
        "pulse_tags": ["脉数"],
        "therapy": "清胃泄热，生津润燥",
        "formula": "玉女煎加减",
        "mods": ["口渴重加天花粉、石斛", "便干重加玄参、火麻仁", "胃火盛加黄连、知母"],
    },
    "气血两虚": {
    ...
    "痰湿阻肺": {
        "symptoms": ["胸闷", "痰多", "咳嗽", "气短", "纳差"],
        "tongue_tags": ["苔白腻", "舌胖", "苔腻"],
        "pulse_tags": ["脉滑"],
        "therapy": "燥湿化痰，宣肺理气",
        "formula": "二陈汤合三子养亲汤加减",
        "mods": ["痰多重加瓜蒌、胆南星", "胸闷重加枳壳、厚朴", "纳差重加砂仁、焦三仙"],
    },
}
